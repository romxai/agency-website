...
// Animated Nav Link Component (no changes needed)
  ...
  return (
    <AnimatePresence>
      <motion.header
        ...
        className={cn(
          "fixed top-2 sm:top-4 md:top-6 left-0 right-0 z-50 flex justify-center px-2 sm:px-4",
          className
        )}
      >
        {/* --- MODIFICATION START --- */}
        {/* Wrapper for positioning the absolute menu panel */}
        <div className="w-full max-w-4xl relative">
          {/* 1. The Main Navigation Bar (Pill) */}
          <div
            className={cn(
              `flex items-center justify-between w-full
               backdrop-blur-sm border border-[#333] bg-[#1f1f1f57]
               transition-all duration-300 ease-in-out
               py-2 sm:py-3 pl-3 pr-3 sm:pl-4 sm:pr-4 md:pl-6 md:pr-6`,
          ...
          >
            ...
            <div className="hidden sm:flex items-center gap-1 sm:gap-2 md:gap-3">
              {buttonItems.map((item, idx) =>
                item.isSignup ? (
                  <div key={`signup-${idx}`} className="relative group">
          ...
          {/* 2. The Absolutely Positioned Mobile Menu Panel */}
          <AnimatePresence>
            {isOpen && (
              <motion.div
                ...
                className="sm:hidden absolute top-full mt-2 w-full p-4
                           backdrop-blur-sm border border-[#333] bg-[#1f1f1f57]
                           rounded-2xl"
              >
